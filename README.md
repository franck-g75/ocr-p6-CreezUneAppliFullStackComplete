...
* Java 21 (le JDK d'oracle) installé sur le PC ( pour le back end )
* Maven 3.9.11
* Angular 20 pour le front end (Angular CLI version 20.3.26)
...
* Créer deux variables d'environnements : (1 pour le login et 1 pour le mot de passe de la base de données)
  ...
  * créer la variable : spring_datasource_username, avec comme valeur le login d'un compte ayant acès à la base de données mdd créée précédemmment.
  * créer la variable : spring_datasource_password, avec comme valeur le mot de passe du compte précédent.
...
* Démarrer Mysql : (si ce n'est pas déjà fait)
  * chercher services dans le moteur de cherche de la barre d'outil puis 
  * chercher Mysql 
  * la fenetre s'ouvre.
  * puis cliquer sur le triangle pour allumer le service 
...
* Dans une autre fenêtre de commande faire cd C:\projet-franck-g75\front
...
* Exécuter la commande `cd C:\projet-franck-g75\back`
* Exécuter la commande `mvn clean site`
* Retrouver la documentation du projet dans C:\projet-franck-g75\back\target\site
